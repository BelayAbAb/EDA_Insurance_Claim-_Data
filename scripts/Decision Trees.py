...
def plot_decision_tree(tree_model, feature_names, output_path):
        ...
        if isinstance(model, DecisionTreeRegressor) or isinstance(model, RandomForestRegressor):
            ...
            if X_train.shape[1] > 0:
                ...
                plot_feature_importance(feature_importances, feature_names,
                                        f'C:\\Users\\User\\Desktop\\10Acadamy\\Week-3\\Data\\MachineLearningRating_v3\\{model_name}_feature_importance.jpg')
            else:
                print("No features to display importance for.")

            # Plot decision tree for single tree models
            if isinstance(model, DecisionTreeRegressor):
                if X_train.shape[1] > 0:
                    plot_decision_tree(model, feature_names,
                                       f'C:\\Users\\User\\Desktop\\10Acadamy\\Week-3\\Data\\MachineLearningRating_v3\\{model_name}_decision_tree.jpg')
                else:
                    print("No features to display in the decision tree.")
    except Exception as e:
        print(f"Error during {model_name} training or evaluation: {e}")
...
def main():
    ...
    try:
        ...
        if X_train is not None and y_train is not None:
            ...
            train_and_evaluate(cart_model, X_train, X_test, y_train, y_test, "CART")
            ...
            plt.figure(figsize=(10, 6))
            plt.scatter(y_test, rf_model.predict(X_test), color='blue', label='Predicted vs Actual')
            plt.xlabel('Actual Values')
            plt.ylabel('Predicted Values')
            plt.title('Random Forest: Actual vs. Predicted Values')
            plt.legend()
            plt.grid(True)
            plt.savefig(r'C:\Users\User\Desktop\10Acadamy\Week-3\Data\MachineLearningRating_v3\random_forest_results.jpg', format='jpg')
            plt.close()
